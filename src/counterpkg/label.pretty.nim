import
  nimgame2 / [textgraphic, entity], assets

when isMainModule:
  import
    nimgame2 / [nimgame, scene, types, graphic, input, settings],
    nimgame2 / gui / widget, core, helpers, circle_graphic, frame
  ...
  proc newTextEntityScene(): Scene =
    ...
    proc newEnt(text: string; pos: var Coord; rot: Angle = 0.0; scale: Scale = 1.0) =
      ...
      fEnt.logic = proc (ent: Entity; dt: float) =
        let color =
          if ent.state.isFocused: ColorOrange
          else: ColorGreen
